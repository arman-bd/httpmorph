...
class TestChrome143Fingerprint:
    """Test Chrome 143 fingerprint accuracy

    Target Chrome 143 fingerprint:
    - JA3N: dcefaf3f0e71d260d19dc1d0749c9278
    - JA3N_FULL: 772,4865-4866-4867-49195-49199-49196-49200-52393-52392-49171-49172-156-157-47-53,0-5-10-11-13-16-18-23-27-35-43-45-51-17613-65037-65281,4588-29-23-24,0
    ...
    def test_chrome143_profile_alias(self):
        """Test that 'chrome' defaults to 'chrome143'"""
        # Both should create valid sessions
        session_chrome = httpmorph.Session(browser="chrome")
        session_chrome143 = httpmorph.Session(browser="chrome143")

        assert session_chrome is not None
        assert session_chrome143 is not None

    def test_chrome143_user_agent(self, httpbin_host):
        """Test Chrome 143 User-Agent is correct"""
        ...
        # User-Agent should contain Chrome/143.0.0.0
        # Note: Can't directly access headers from response, but session uses correct UA

    def test_chrome143_ja3n_consistency(self, httpbin_host):
        """Test JA3N (normalized) fingerprint is consistent

        Expected JA3N: dcefaf3f0e71d260d19dc1d0749c9278
        ...
        ja3n_hashes = []
        for _ in range(3):
            response = httpmorph.get(f"https://{httpbin_host}", browser="chrome143")
    ...
    def test_chrome143_tls_version(self, httpbin_host):
        """Test Chrome 143 uses TLS 1.2/1.3"""
        response = httpmorph.get(f"https://{httpbin_host}", browser="chrome143")
    ...
    def test_chrome143_cipher_suite(self, httpbin_host):
        """Test Chrome 143 cipher suite selection"""
        response = httpmorph.get(f"https://{httpbin_host}", browser="chrome143")
    ...
    def test_chrome143_http2_support(self, httpbin_host):
        """Test Chrome 143 HTTP/2 support with JA4 characteristics
        ...
        - h2: HTTP/2 support
        """
        session = httpmorph.Session(browser="chrome143", http2=True)
        response = session.get(f"https://{httpbin_host}/get", timeout=10)

        # Chrome 143 supports HTTP/2
        assert response.status_code in [200, 402]
        assert response.http_version in ["1.1", "2.0"]

    def test_chrome143_post_quantum_crypto(self, httpbin_host):
        """Test Chrome 143 includes post-quantum cryptography support

        Chrome 143 includes X25519MLKEM768 (curve 4588/0x11ec) in supported curves.
        This is part of the JA3N_FULL: 4588-29-23-24
        """
        # Create session with Chrome 143 profile
        session = httpmorph.Session(browser="chrome143")
        response = session.get(f"https://{httpbin_host}")
    ...
    def test_chrome_alias_equals_chrome143(self, httpbin_host):
        """Test that 'chrome' and 'chrome143' produce identical fingerprints"""
        # Make requests with both aliases
        session_chrome = httpmorph.Session(browser="chrome")
        session_143 = httpmorph.Session(browser="chrome143")

        response_chrome = session_chrome.get(f"https://{httpbin_host}")
        response_143 = session_143.get(f"https://{httpbin_host}")

        # Both should succeed
        assert response_chrome.status_code in [200, 402]
        assert response_143.status_code in [200, 402]

        # Both should use same TLS version and cipher
        assert response_chrome.tls_version == response_143.tls_version
        assert response_chrome.tls_cipher == response_143.tls_cipher
    ...
    def test_os_macos_user_agent(self, httpbin_host):
        ...
        if response.status_code == 200:
            ...
            assert "Chrome/143.0.0.0" in response_text
    ...
    def test_os_windows_user_agent(self, httpbin_host):
        ...
        session = httpmorph.Session(browser="chrome143", os="windows")
        response = session.get(f"https://{httpbin_host}/user-agent")
    ...
    def test_os_linux_user_agent(self, httpbin_host):
        ...
        session = httpmorph.Session(browser="chrome143", os="linux")
        response = session.get(f"https://{httpbin_host}/user-agent")
