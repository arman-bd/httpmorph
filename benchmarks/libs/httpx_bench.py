...
class HttpxBenchmark(LibraryBenchmark):
        ...
        async def request():
            try:
                async with httpx.AsyncClient(
                    proxy=self.proxy_url_http, verify=False, timeout=10
                ) as client:
    ...
    def async_proxy_https(self):
        ...
        async def request():
            try:
                async with httpx.AsyncClient(
                    proxy=self.proxy_url_https, verify=False, timeout=10
                ) as client:
    ...
    def async_proxy_http2(self):
        ...
        async def request():
            try:
                async with httpx.AsyncClient(
                    http2=True, proxy=self.proxy_url_https, verify=False, timeout=10
                ) as client:
                    resp = await client.get(self.proxy_target_https)
                    assert 200 <= resp.status_code < 600, f"Got status {resp.status_code}"
            except Exception:
