...
class AiohttpBenchmark(LibraryBenchmark):
    ...
    def async_local_http(self):
        async def request():
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
                async with session.get(self.local_url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
    ...
    def async_remote_http(self):
        async def request():
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
                async with session.get(self.remote_http_url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    assert 200 <= resp.status < 600, f"Got status {resp.status}"
                    await resp.read()
        return self.run_async_benchmark(request)
    ...
    def async_remote_https(self):
        async def request():
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
                async with session.get(self.remote_https_url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    assert 200 <= resp.status < 600, f"Got status {resp.status}"
                    await resp.read()
        return self.run_async_benchmark(request)
    ...
    def async_proxy_https(self):
        ...
        async def request():
            try:
                async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
                    async with session.get(self.proxy_target_https, proxy=self.proxy_url_https, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                        assert 200 <= resp.status < 600, f"Got status {resp.status}"
                        await resp.read()
            except Exception:
                pass
