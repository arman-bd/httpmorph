...
class LibraryBenchmark(ABC):
    ...
    def run_sequential_benchmark(self, name, func):
        ...
        try:
            ...
            cv = (statistics.stdev(times) / mean_val * 100) if mean_val > 0 and len(times) > 1 else 0
    ...
    def run_concurrent_sync(self, func):
        ...
        try:
            ...
            try:
                ...
                numerator = sum((x_vals[i] - x_mean) * (completion_times[i] - y_mean) for i in range(n))
                denominator = sum((x_vals[i] - x_mean) ** 2 for i in range(n))
            ...
            cv = (statistics.stdev(completion_times) / mean_val * 100) if mean_val > 0 and len(completion_times) > 1 else 0
            ...
            return {
                ...
                'p95_completion_ms': completion_times[int(n * 0.95)] if n > 20 else max(completion_times) if completion_times else 0,
