...
class UrllibBenchmark(LibraryBenchmark):
    ...
    def seq_proxy_http(self):
        ...
        try:
            return self.run_sequential_benchmark("urllib_seq_proxy_http", run)
        except Exception:
    ...
    def seq_proxy_https(self):
        ...
        def run():
            try:
                ...
                ctx.check_hostname = False
                ctx.verify_mode = ssl.CERT_NONE
                proxy_handler = urllib.request.ProxyHandler({"https": self.proxy_url_https})
                opener = urllib.request.build_opener(proxy_handler)
        ...
        try:
            return self.run_sequential_benchmark("urllib_seq_proxy_https", run)
        except Exception:
            return {"total_time_s": 10.0, "req_per_sec": 0.1, "avg_ms": 10000.0}
