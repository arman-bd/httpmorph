...
class CurlCffiBenchmark(LibraryBenchmark):
    ...
    def seq_proxy_http(self):
        ...
        def run():
            ...
            resp = curl_requests.get(self.proxy_target_http, proxies=proxies, verify=False, timeout=10)
            assert 200 <= resp.status_code < 600, f"Got status {resp.status_code}"
    ...
    def seq_proxy_https(self):
        ...
        def run():
            ...
            resp = curl_requests.get(self.proxy_target_https, proxies=proxies, verify=False, timeout=10)
            assert 200 <= resp.status_code < 600, f"Got status {resp.status_code}"
